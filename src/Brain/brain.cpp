...
array<float, OUTPUT_SIZE> Brain::forward(const vector<float>& input_list, const array<Neuron_link, GENOME_LENGHT>& neuron_links)
{
...
float Brain::calculate_output(const std::vector<float>& input_list, const std::array<Neuron_link, GENOME_LENGHT>& neuron_links, int index, NEURON_TYPE neuron_type)
{
    float result = 0.;
    vector<float> output;
    ...
    for (int i = 0; i < GENOME_LENGHT; i++) {
        const Neuron_link& link = neuron_links[i];
    ...
    if (neuron_type == LAYER_NEURON) {
        ...
        for (const float out : output)
            result += out;
        result /= output.size() * DEFAULT_OUTPUT_SIZE;
